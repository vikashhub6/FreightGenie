...
const { generatePDFReport, saveAnalyticsCSV } = require("../services/pdfService");
...
exports.approveReport = async (req, res) => {
  try {
    ...
    // ── Phase 2: Save analytics to MongoDB ──
    ...
    shipment.analytics = {
      ...
      shippingMode:     si.shippingMode  || "sea",
      cargoType:        shipment.cargoType,
      hsCode:           r.hsCode || ed.hsCode || "",
      grossWeight:      ed.grossWeight || "",
      volume:           ed.volume      || "",
      ...
      shipmentMonth:    now.toLocaleString("en-US", { month: "long" }),
      shipmentYear:     now.getFullYear(),
      savedAt:          now,
    };

    await shipment.save();

    // ── Phase 2: Auto-save analytics CSV ──
    saveAnalyticsCSV(shipment);
