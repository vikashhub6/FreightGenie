...
const output_js_1 = require("./schema/output.cjs");
const language_model_js_1 = require("./schema/language_model.cjs");
 ...
 */
class BaseChatModel extends language_model_js_1.BaseLanguageModel {
    constructor(fields) {
        super(fields);
    ...
    async _generateUncached(messages, parsedOptions, handledOptions) {
        const baseMessages = messages.map((messageList) => messageList.map(messages_js_1.coerceMessageLikeToMessage));
        ...
        Object.defineProperty(output, output_js_1.RUN_KEY, {
            value: runManagers
    ...
    async call(messages, options, callbacks) {
        const result = await this.generate([messages.map(messages_js_1.coerceMessageLikeToMessage)], options, callbacks);
    ...
    async predict(text, options, callbacks) {
        const message = new messages_js_1.HumanMessage(text);
...
class SimpleChatModel extends BaseChatModel {
    async _generate(messages, options, runManager) {
        ...
        const message = new messages_js_1.AIMessage(text);
