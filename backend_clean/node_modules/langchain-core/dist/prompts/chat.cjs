...
const messages_js_1 = require("../schema/messages.cjs");
const messages_js_2 = require("../schema/messages.cjs");
const prompt_js_1 = require("../schema/prompt.cjs");
const base_js_1 = require("../runnables/base.cjs");
const prompt_template_js_1 = require("../schema/prompt_template.cjs");
 ...
 */
class ChatPromptValue extends prompt_js_1.BasePromptValue {
    static lc_name() {
    ...
    toString() {
        return (0, messages_js_1.getBufferString)(this.messages);
    }
    toChatMessages() {
...
class MessagesPlaceholder extends BaseMessagePromptTemplate {
    ...
    validateInputOrThrow(input, variableName) {
        ...
        if (Array.isArray(input)) {
            isInputBaseMessage = input.every((message) => (0, messages_js_2.isBaseMessage)(message));
        }
        else {
            isInputBaseMessage = (0, messages_js_2.isBaseMessage)(input);
        }
 ...
 */
class BaseChatPromptTemplate extends prompt_template_js_1.BasePromptTemplate {
    constructor(input) {
        super(input);
    }
    async format(values) {
...
class ChatMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new messages_js_2.ChatMessage(await this.prompt.format(values), this.role);
    }
    static fromTemplate(template, role) {
        return new this(prompt_js_2.PromptTemplate.fromTemplate(template), role);
    }
}
...
class HumanMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new messages_js_2.HumanMessage(await this.prompt.format(values));
    }
    static fromTemplate(template) {
        return new this(prompt_js_2.PromptTemplate.fromTemplate(template));
    }
}
...
class AIMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new messages_js_2.AIMessage(await this.prompt.format(values));
    }
    static fromTemplate(template) {
        return new this(prompt_js_2.PromptTemplate.fromTemplate(template));
    }
...
class SystemMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new messages_js_2.SystemMessage(await this.prompt.format(values));
    }
    static fromTemplate(template) {
        return new this(prompt_js_2.PromptTemplate.fromTemplate(template));
    }
...
function _coerceMessagePromptTemplateLike(messagePromptTemplateLike) {
    if (_isBaseMessagePromptTemplate(messagePromptTemplateLike) ||
        (0, messages_js_2.isBaseMessage)(messagePromptTemplateLike)) {
        return messagePromptTemplateLike;
    }
    const message = (0, messages_js_2.coerceMessageLikeToMessage)(messagePromptTemplateLike);
    ...
    else if (messages_js_2.ChatMessage.isInstance(message)) {
...
class ChatPromptTemplate extends BaseChatPromptTemplate {
    ...
    constructor(input) {
        ...
        if (this.validateTemplate) {
            ...
            for (const promptMessage of this.promptMessages) {
                ...
                if (promptMessage instanceof messages_js_2.BaseMessage)
                    continue;
    ...
    async _parseImagePrompts(message, inputValues) {
        ...
        const formattedMessageContent = await Promise.all(message.content.map(async (item) => {
            ...
            const promptTemplatePlaceholder = prompt_js_2.PromptTemplate.fromTemplate(imageUrl);
     ...
     */
    static fromTemplate(template) {
        const prompt = prompt_js_2.PromptTemplate.fromTemplate(template);
