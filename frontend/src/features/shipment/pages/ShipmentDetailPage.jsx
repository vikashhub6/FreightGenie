...
import { downloadBookingPDFAPI, downloadInvoicePDFAPI } from "../../compliance/services/complianceService";
...
export default function ShipmentDetailPage() {
  ...
  const handleDownloadBooking = async () => {
    setComplianceLoading("booking");
    try {
      const res = await downloadBookingPDFAPI(id);
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/pdf" }));
      const a = document.createElement("a"); a.href = url;
      a.download = `booking-${shipment.shipmentId}.pdf`; a.click();
      URL.revokeObjectURL(url);
    } catch (e) { alert(e.response?.data?.error || "Booking PDF not available yet"); }
    finally { setComplianceLoading(""); }
  };

  const handleDownloadInvoice = async () => {
    setComplianceLoading("invoice");
    try {
      const res = await downloadInvoicePDFAPI(id);
      const url = URL.createObjectURL(new Blob([res.data], { type: "application/pdf" }));
      const a = document.createElement("a"); a.href = url;
      a.download = `invoice-${shipment.shipmentId}.pdf`; a.click();
      URL.revokeObjectURL(url);
    } catch (e) { alert(e.response?.data?.error || "Invoice not available — send email first"); }
    finally { setComplianceLoading(""); }
  };
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      ["Shipping Mode",    shipment.shipmentInfo?.shippingMode?.toUpperCase() || "—"],
                      ["Payment Terms",    shipment.shipmentInfo?.paymentTerms || "—"],
                      ["Port of Loading",  shipment.shipmentInfo?.portOfLoading || "—"],
                      ["Port of Discharge",shipment.shipmentInfo?.portOfDischarge || "—"],
                      ["Consignee",        shipment.shipmentInfo?.consigneeName || "—"],
                      ["Expected Ship",    shipment.shipmentInfo?.expectedShipDate ? new Date(shipment.shipmentInfo.expectedShipDate).toLocaleDateString() : "—"],
                      ["Exporter Email",   shipment.exporterEmail],
                      ["Documents",        shipment.documents?.length || 0],
                    ].map(([k,v]) => (
                  ...
                  {shipment.shipmentInfo?.specialInstructions && (
                    <div className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)" }}>
                      <div className="text-[10px] uppercase tracking-widest font-medium mb-1" style={{ color: "var(--text-muted)" }}>Special Instructions</div>
                      <div className="text-sm" style={{ color: "var(--text-primary)" }}>{shipment.shipmentInfo.specialInstructions}</div>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                {hasPDF && (
                  <button className="btn-outline w-full text-sm" onClick={handleDownloadPDF}
                    ...
                    {complianceLoading === "download" ? "⏳ Downloading..." : "⬇️ Compliance Report PDF"}
                  </button>
                )}

                {/* Booking Confirmation PDF */}
                {shipment.bookingPdfPath && (
                  <button className="btn-outline w-full text-sm" onClick={handleDownloadBooking}
                    disabled={complianceLoading === "booking"}>
                    {complianceLoading === "booking" ? "⏳ Downloading..." : "📋 Booking Confirmation PDF"}
                  </button>
                )}

                {/* Freight Invoice PDF */}
                {shipment.freightInvoicePath && (
                  <button className="btn-outline w-full text-sm" onClick={handleDownloadInvoice}
                    disabled={complianceLoading === "invoice"}>
                    {complianceLoading === "invoice" ? "⏳ Downloading..." : "🧾 Freight Invoice PDF"}
                  </button>
                )}
